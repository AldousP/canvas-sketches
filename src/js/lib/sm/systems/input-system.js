  ...
  this.mapKeys = [];
    ...
    eventMappings.forEach(function (eventName) {
          ...
          if (inputSource) {
            ...
            buttons.forEach(function (button) {
              ...
              if (buttonData && buttonData.pressed) {
                fire(null, eventName, { val: buttonData.value });
              }
            });
          }
        }
          ...
          if (axes && inputSource.axes && inputSource.axes.length) {
            ...
            axesList.forEach(function (axe) {
              switch (axe) {
                case 'leftStick':
                  var config = axes[axe];
                  var leftStick = new SVec.Vector(inputSource.axes[0], -inputSource.axes[1]);
                  var val = leftStick.len;
                  var normVal = SMath.project(val, 0, 1.5, 0, 1);
                  if (normVal > config.deadZone) {
                    fire(null, eventName, { val: leftStick});
                  }
                  break;
                case 'rightStick':
                  var config = axes[axe];
                  var rightStick = new SVec.Vector(inputSource.axes[2], -inputSource.axes[3]);
                  var val = rightStick.len;
                  var normVal = SMath.project(val, 0, 1.5, 0, 1);
                  if (normVal > config.deadZone) {
                    fire(null, eventName, { val: rightStick});
                  }
                  break;
              }
            })
      ...
      if (padConfig) {
        ...
        if (x || y) {
          fire(null, eventName, { val: new SVec.Vector(x, y)});
        }
      }
    });
	};
